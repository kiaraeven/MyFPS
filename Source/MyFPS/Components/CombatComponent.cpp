...
void UCombatComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(UCombatComponent, CarriedAmmo, COND_OwnerOnly); // carried ammo only displayed on the owning client
...
void UCombatComponent::SetAiming(bool bIsAiming)
{
	...
	if (Character->IsLocallyControlled() && EquippedWeapon->GetWeaponType() == EWeaponType::EWT_AssaultRifle)
	{
...
void UCombatComponent::OnRep_EquippedWeapon()
{
	...
	Character->GetCharacterMovement()->bOrientRotationToMovement = false; // Character now rotates based on controller rotation
...
// Equips the specified weapon to the character. This will only be called on the server.
...
void UCombatComponent::Reload() {
	if (CarriedAmmo > 0 && CombatState == ECombatState::ECS_Unoccupied && EquippedWeapon && !EquippedWeapon->AmmoIsFull() && !bLocallyReloading)
	{
		ServerReload();
		HandleReload();
